...
class StudentsResourceIT {
    ...
    void getAllStudentsWithEagerRelationshipsIsEnabled() throws Exception {
        when(studentsRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restStudentsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(studentsRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllStudentsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restStudentsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
