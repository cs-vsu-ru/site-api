...
class ScientificWorkTypeResourceIT {
    ...
    void getAllScientificWorkTypesWithEagerRelationshipsIsEnabled() throws Exception {
        when(scientificWorkTypeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restScientificWorkTypeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(scientificWorkTypeRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllScientificWorkTypesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(scientificWorkTypeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restScientificWorkTypeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(scientificWorkTypeRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
