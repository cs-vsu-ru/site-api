...
// private static final Instant DEFAULT_DATE_OF_BIRTH =
// Instant.ofEpochMilli(0L);
// private static final Instant UPDATED_DATE_OF_BIRTH =
...
// Employee().patronymic(DEFAULT_PATRONYMIC).dateOfBirth(DEFAULT_DATE_OF_BIRTH);
// // Add required entity
...
// Employee employee = new
// Employee().patronymic(UPDATED_PATRONYMIC).dateOfBirth(UPDATED_DATE_OF_BIRTH);
// // Add required entity
// User user = UserResourceIT.createEntity(em);
// em.persist(user);
// em.flush();
...
// when(employeeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new
// PageImpl(new ArrayList<>()));

// restEmployeeMockMvc.perform(get(ENTITY_API_URL +
...
// // Get the employee
// restEmployeeMockMvc.perform(get(ENTITY_API_URL_ID,
...
// employee.getId()).accept(MediaType.APPLICATION_JSON))
