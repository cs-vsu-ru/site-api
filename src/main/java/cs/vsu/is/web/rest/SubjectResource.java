...
import cs.vsu.is.service.SubjectService;
...
/**
 * REST controller for managing {@link cs.vsu.is.domain.Subject}.
 */
@RestController
@RequestMapping("/api")
public class SubjectResource {
   ...
   *         body the new subjectDTO, or with status {@code 400 (Bad Request)} if
   *         the subject has already an ID.
   ...
   *         the updated subjectDTO,
   *         or with status {@code 400 (Bad Request)} if the subjectDTO is not
   *         valid,
   *         or with status {@code 500 (Internal Server Error)} if the subjectDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  public ResponseEntity<SubjectDTO> updateSubject(
      ...
      @RequestBody SubjectDTO subjectDTO) throws URISyntaxException {
  ...
  /**
   * {@code PATCH  /subjects/:id} : Partial updates given fields of an existing
   * subject, field will ignore if it is null
   *
   * @param id         the id of the subjectDTO to save.
   * @param subjectDTO the subjectDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   ...
   *         or with status {@code 400 (Bad Request)} if the subjectDTO is not
   *         valid,
   *         or with status {@code 404 (Not Found)} if the subjectDTO is not
   *         found,
   *         or with status {@code 500 (Internal Server Error)} if the subjectDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  // @PatchMapping(value = "/subjects/{id}", consumes = { "application/json",
  // "application/merge-patch+json" })
  // public ResponseEntity<SubjectDTO> partialUpdateSubject(
  // @PathVariable(value = "id", required = false) final Long id,
  // @RequestBody SubjectDTO subjectDTO
  // ) throws URISyntaxException {
  // log.debug("REST request to partial update Subject partially : {}, {}", id,
  // subjectDTO);
  // if (subjectDTO.getId() == null) {
  // throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
  // }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   *         the subjectDTO, or with status {@code 404 (Not Found)}.
   */
