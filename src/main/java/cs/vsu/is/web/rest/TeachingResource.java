...
public class TeachingResource {
   ...
   *         body the new teachingDTO, or with status {@code 400 (Bad Request)} if
   *         the teaching has already an ID.
   ...
   *         the updated teachingDTO,
   *         or with status {@code 400 (Bad Request)} if the teachingDTO is not
   *         valid,
   *         or with status {@code 500 (Internal Server Error)} if the teachingDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  public ResponseEntity<TeachingDTO> updateTeaching(
      ...
      @RequestBody TeachingDTO teachingDTO) throws URISyntaxException {
  ...
  /**
   * {@code PATCH  /teachings/:id} : Partial updates given fields of an existing
   * teaching, field will ignore if it is null
   *
   * @param id          the id of the teachingDTO to save.
   * @param teachingDTO the teachingDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   ...
   *         or with status {@code 400 (Bad Request)} if the teachingDTO is not
   *         valid,
   *         or with status {@code 404 (Not Found)} if the teachingDTO is not
   *         found,
   *         or with status {@code 500 (Internal Server Error)} if the teachingDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  // @PatchMapping(value = "/teachings/{id}", consumes = { "application/json",
  // "application/merge-patch+json" })
  // public ResponseEntity<TeachingDTO> partialUpdateTeaching(
  // @PathVariable(value = "id", required = false) final Long id,
  // @RequestBody TeachingDTO teachingDTO
  // ) throws URISyntaxException {
  // log.debug("REST request to partial update Teaching partially : {}, {}", id,
  // teachingDTO);
  // if (teachingDTO.getId() == null) {
  // throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
  // }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   *         the teachingDTO, or with status {@code 404 (Not Found)}.
   */
