...
public class ScheduleResource {
   ...
   *         body the new scheduleDTO, or with status {@code 400 (Bad Request)} if
   *         the schedule has already an ID.
   ...
   *         or with status {@code 400 (Bad Request)} if the scheduleDTO is not
   *         valid,
   *         or with status {@code 500 (Internal Server Error)} if the scheduleDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  public ResponseEntity<ScheduleDTO> updateSchedule(
      ...
      @RequestBody ScheduleDTO scheduleDTO) throws URISyntaxException {
  ...
  /**
   * {@code PATCH  /schedules/:id} : Partial updates given fields of an existing
   * schedule, field will ignore if it is null
   *
   * @param id          the id of the scheduleDTO to save.
   * @param scheduleDTO the scheduleDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   ...
   *         or with status {@code 400 (Bad Request)} if the scheduleDTO is not
   *         valid,
   *         or with status {@code 404 (Not Found)} if the scheduleDTO is not
   *         found,
   *         or with status {@code 500 (Internal Server Error)} if the scheduleDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  // @PatchMapping(value = "/schedules/{id}", consumes = { "application/json",
  // "application/merge-patch+json" })
  // public ResponseEntity<ScheduleDTO> partialUpdateSchedule(
  // @PathVariable(value = "id", required = false) final Long id,
  // @RequestBody ScheduleDTO scheduleDTO
  // ) throws URISyntaxException {
  // log.debug("REST request to partial update Schedule partially : {}, {}", id,
  // scheduleDTO);
  // if (scheduleDTO.getId() == null) {
  // throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
  // }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   *         the scheduleDTO, or with status {@code 404 (Not Found)}.
   */
