...
import cs.vsu.is.service.RoleService;
...
/**
 * REST controller for managing {@link cs.vsu.is.domain.Role}.
 */
@RestController
@RequestMapping("/api")
public class RoleResource {
   ...
   *         or with status {@code 500 (Internal Server Error)} if the roleDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/roles/{id}")
  public ResponseEntity<RoleDTO> updateRole(@PathVariable(value = "id", required = false) final Long id,
   ...
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  // @PatchMapping(value = "/roles/{id}", consumes = { "application/json",
  // "application/merge-patch+json" })
  // public ResponseEntity<RoleDTO> partialUpdateRole(
  // @PathVariable(value = "id", required = false) final Long id,
  // @RequestBody RoleDTO roleDTO
  // ) throws URISyntaxException {
  // log.debug("REST request to partial update Role partially : {}, {}", id,
  // roleDTO);
  // if (roleDTO.getId() == null) {
  // throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
  // }
