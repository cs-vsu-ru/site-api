...
public class ArticlesResource {
   ...
   *         body the new articlesDTO, or with status {@code 400 (Bad Request)} if
   *         the articles has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/articles")
  public ResponseEntity<ArticleDTO> createArticles(@RequestBody ArticleDTO articlesDTO) throws URISyntaxException {
    ...
    ArticleDTO result = articlesService.save(articlesDTO);
   ...
   *         the updated articlesDTO,
   *         or with status {@code 400 (Bad Request)} if the articlesDTO is not
   *         valid,
   *         or with status {@code 500 (Internal Server Error)} if the articlesDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/articles/{id}")
  public ResponseEntity<ArticleDTO> updateArticles(
      @PathVariable(value = "id", required = false) final Long id,
      @RequestBody ArticleDTO articlesDTO) throws URISyntaxException {
    ...
    ArticleDTO result = articlesService.update(articlesDTO);
    return ResponseEntity
        .ok()
        .headers(
   ...
   * articles, field will ignore if it is null
   *
   * @param id          the id of the articlesDTO to save.
   * @param articlesDTO the articlesDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   ...
   *         or with status {@code 400 (Bad Request)} if the articlesDTO is not
   *         valid,
   *         or with status {@code 404 (Not Found)} if the articlesDTO is not
   *         found,
   *         or with status {@code 500 (Internal Server Error)} if the articlesDTO
   *         couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  // @PatchMapping(value = "/articles/{id}", consumes = { "application/json",
  // "application/merge-patch+json" })
  // public ResponseEntity<ArticleDTO> partialUpdateArticles(
  // @PathVariable(value = "id", required = false) final Long id,
  // @RequestBody ArticleDTO articlesDTO
  // ) throws URISyntaxException {
  // log.debug("REST request to partial update Articles partially : {}, {}", id,
  // articlesDTO);
  // if (articlesDTO.getId() == null) {
  // throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
  // }
  ...
  // Optional<ArticleDTO> result = articlesService.partialUpdate(articlesDTO);

  // return ResponseUtil.wrapOrNotFound(
  // result,
  // HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME,
  // articlesDTO.getId().toString())
  // );
  // }
  ...
  public ResponseEntity<ArticleDTO> getArticles(@PathVariable Long id) {
    ...
    Optional<ArticleDTO> articlesDTO = articlesService.findOne(id);
